...
# பிரீமியம் லேஅவுட் வடிவமைப்பு
st.set_page_config(page_title="PCAS VIRTUAL OFFICE", page_icon="🛡️", layout="wide")
...
st.markdown("""
<style>
    /* 🏛️ 🔵 மெயின் ஹெடருக்கான புதிய Mild Color பாக்ஸ் */
    .header-mild-box {
        background-color: #ebf5fb !important; /* மிகவும் மென்மையான ஸ்கை ப்ளூ */
    ...
    /* 📏 ஆபீஸ் பெயர் கச்சிதமான அளவில் நேர்க்கோட்டில் அலைன் செய்யப்பட்டுள்ளது */
    .main-title {
        ...
        margin: 0 !important;
        padding: 0 !important;
        line-height: 75px !important; /* லோகோவின் உயரத்திற்கு இணையாக சென்டர் செய்ய */
    }
    ...
    .desk-title { font-size: 13px; font-weight: bold; color: #2c3e50; margin-bottom: 5px; }
    ...
    .notice-box { background-color: #fff3cd; border: 1px solid #ffeba2; border-left: 6px solid #ffc107; border-radius: 6px; padding: 15px; margin-bottom: 20px; }
    ...
    .msg-out { background-color: #d5f5e3; border-radius: 10px; padding: 8px 12px; margin: 5px 0; max-width: 85%; float: right; clear: both; box-shadow: 1px 1px 2px rgba(0,0,0,0.1); }
    .msg-in { background-color: #ffffff; border-radius: 10px; padding: 8px 12px; margin: 5px 0; max-width: 85%; float: left; clear: both; border: 1px solid #e5e7eb; box-shadow: 1px 1px 2px rgba(0,0,0,0.05); }
</style>
...
if not st.session_state.logged_in:
    ...
    with col_l2:
        with st.form(key="login_form"):
            st.image("pcas_logo.png", width=140)
            st.markdown(
                '<h3 style="color: #1a365d; font-family: sans-serif; margin-top: -10px;">Secure Login Gate</h3>',
                unsafe_allow_html=True)
...
if len(all_messages) > st.session_state.msg_count:
    st.session_state.msg_count = len(all_messages)
    st.markdown(
        '<iframe src="https://assets.mixkit.co/active_storage/sfx/2869/2869-500.wav" allow="autoplay" style="display:none;"></iframe>',
        unsafe_allow_html=True)
...
# 🏛️ 💎 லோகோ + பெயர் எர்ரர் இல்லாமல் கச்சிதமாக லோடாக புதிய எளிய செட்டப்
st.markdown('<div class="header-mild-box">', unsafe_allow_html=True)
col_logo_native, col_title_native = st.columns([0.06, 0.94])
with col_logo_native:
    st.image("pcas_logo.png", width=75)
...
# ----------------------------------------------------
st.markdown(
    f'<div class="notice-box">📢 <b>OFFICE NOTICE BOARD:</b> "{notice_board["text"]}" <span style="float:right; color:#666; font-size:12px;">- Posted by {notice_board["by"]}</span></div>',
    unsafe_allow_html=True)
...
for idx, data in office_desks.items():
    if data["name"] != "🪑 Empty":
        ...
        if "Online" in data["status"]:
            online_count += 1
        elif "Busy" in data["status"]:
            busy_count += 1
        elif "Break" in data["status"]:
            break_count += 1
        elif "WFH" in data["status"]:
            wfh_count += 1

st.markdown("### 📊 Live Management Overview")
m_col1, m_col2, m_col3, m_col4, m_col5 = st.columns(5)
with m_col1:
    st.markdown(f'<div class="metric-card"><b style="color:#2ecc71;">🟢 Online Staff</b><h2>{online_count}</h2></div>',
                unsafe_allow_html=True)
with m_col2:
    st.markdown(f'<div class="metric-card"><b style="color:#e74c3c;">🔴 Busy / Meeting</b><h2>{busy_count}</h2></div>',
                unsafe_allow_html=True)
with m_col3:
    st.markdown(f'<div class="metric-card"><b style="color:#f1c40f;">🟡 On Break</b><h2>{break_count}</h2></div>',
                unsafe_allow_html=True)
with m_col4:
    st.markdown(f'<div class="metric-card"><b style="color:#3498db;">🔵 WFH Staff</b><h2>{wfh_count}</h2></div>',
                unsafe_allow_html=True)
with m_col5:
    st.markdown(
        f'<div class="metric-card"><b style="color:#1a365d;">👥 Total Active</b><h2>{len(active_staff)}</h2></div>',
        unsafe_allow_html=True)
...
with col_control:
    ...
    for i in range(1, 23):
        if 1 <= i <= 3:
            ...
            dept = "Mechanical Team"
        elif 10 <= i <= 14:
            dept = "Electrical Team"
        elif 15 <= i <= 17:
    ...
    with st.form(key="group_chat_form", clear_on_submit=True):
        ...
        uploaded_file = st.file_uploader("Share Photo or File to Everyone 📎",
                                         type=["png", "jpg", "jpeg", "pdf", "txt", "xlsx"], key="group_file")
            ...
            if msg["receiver"] == "Everyone":
                ...
                if msg.get("file") is not None:
                    if msg["file_name"].lower().endswith(('.png', '.jpg', '.jpeg')):
                        ...
                        st.download_button(label=f"📥 Download {msg['file_name']}", data=msg["file"],
                                           file_name=msg["file_name"], key=f"grp_dl_{msg['time']}_{msg['sender']}")
    st.markdown('</div>', unsafe_allow_html=True)
    ...
    if st.session_state.chat_with_user:
        ...
        if target_user != my_name:
            ...
            with st.form(key=f"private_form_{target_user}", clear_on_submit=True):
                ...
                p_uploaded_file = st.file_uploader(f"Send Photo/File to {target_user} 📎",
                                                   type=["png", "jpg", "jpeg", "pdf", "txt", "xlsx"],
                                                   key=f"p_file_{target_user}")
                ...
                if (msg["sender"] == my_name and msg["receiver"] == target_user):
                    ...
                    if msg.get("file") is not None:
                        if msg["file_name"].lower().endswith(('.png', '.jpg', '.jpeg')):
                            ...
                            st.download_button(label=f"📥 Download {msg['file_name']}", data=msg["file"],
                                               file_name=msg["file_name"], key=f"p_dl_out_{idx}")
                    has_messages = True
                    ...
                    if msg.get("file") is not None:
                        if msg["file_name"].lower().endswith(('.png', '.jpg', '.jpeg')):
                            ...
                            st.download_button(label=f"📥 Download {msg['file_name']}", data=msg["file"],
                                               file_name=msg["file_name"], key=f"p_dl_in_{idx}")
                    has_messages = True

            if not has_messages:
                st.caption("No private messages yet. Say hello!")
...
with col_floor_plan:
    ...
    st.link_button("🚨 JOIN OFFICE GROUP CALL (ALL ONLINE STAFF)", COMMON_GROUP_MEET_URL, type="primary",
        ...
        for idx, d_num in enumerate(desk_range):
            ...
            with cols[idx]:
                if d_data["name"] == "🪑 Empty":
                    st.markdown(f'<div class="status-offline"><div class="desk-title">Desk {d_num}</div>🪑 Empty</div>',
                                unsafe_allow_html=True)
                    st.write("")
                else:
                    if "Online" in d_data["status"]:
                        style_class = "status-online"
                    elif "Busy" in d_data["status"]:
                        style_class = "status-busy"
                    elif "Break" in d_data["status"]:
                        style_class = "status-break"
                    elif "WFH" in d_data["status"]:
                        style_class = "status-wfh"
                    else:
                        style_class = "status-offline"

                    st.markdown(
                        f'<div class="{style_class}"><div class="desk-title">Desk {d_num}</div>👤 <b>{d_data["name"]}</b><br>{d_data["status"]}<br><span style="font-size:10px; color:#555;">🕒 {d_data.get("checkin_time", "-")}</span></div>',
                        unsafe_allow_html=True)
                    st.link_button("🎛️ Connect Call", "https://meet.google.com/new", key=f"lnk_{d_num}",
                                   use_container_width=True)
